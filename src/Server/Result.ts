...
export default class Result {
	...
	constructor(response: any, description?: string, code?: number, res?: Response) {
		let version: string | string[] | null = String(global.URL || "").match(/(v[0-9\.]+)/gi);
		...
		this.status = (code_list as any)[String(this.code) as string] as string;
		...
		try {
			start_time = new Date((global as any).startRequest as any);
			end_time = new Date();
